...
display_menu(){
    ...
    if [[ "$default" != "" ]]; then
        ...
        default_prompt="(默认 ${arr[$default-1]})"
    fi
    local pick
    local hint
    local vname
    local prompt="您想选择哪个 ${soft} ${default_prompt}: "
...
get_latest_version() {
    ...
    [ -z "${deb_name}" ] && _error "获取 Linux 内核二进制包名失败，可能是内核构建失败。请选择其他版本并重试。"
}
...
check_os() {
    ...
    if [ -n "${virt}" -a "${virt}" = "lxc" ]; then
        _error "虚拟化方式为 LXC，不支持。"
    fi
    if [ -n "${virt}" -a "${virt}" = "openvz" ] || [ -d "/proc/vz" ]; then
        _error "虚拟化方式为 OpenVZ，不支持。"
    fi
        ...
        centos)
            if [ -n "$(_os_ver)" ]; then
                ...
                if [ "$(_os_ver)" -eq 6 ]; then
                    ...
                    [ -s "${rpm_kernel_name}" ] && _error_detect "rpm -ivh ${rpm_kernel_name}" || _error "下载 ${rpm_kernel_name} 失败，请检查。"
                    [ -s "${rpm_kernel_devel_name}" ] && _error_detect "rpm -ivh ${rpm_kernel_devel_name}" || _error "下载 ${rpm_kernel_devel_name} 失败，请检查。"
                    rm -f ${rpm_kernel_name} ${rpm_kernel_devel_name}
                    [ ! -f "/boot/grub/grub.conf" ] && _error "/boot/grub/grub.conf 未找到，请检查。"
...
install_bbr() {
    if check_bbr_status; then
        echo
        _info "脚本已成功安装 BBR 并更新所有设置。"
	sleep 5
        exit 0
    fi
    if check_kernel_version; then
        echo
        _info "内核版本大于 4.9，直接设置 TCP BBR..."
        sysctl_config
        _info "设置 TCP BBR 完成..."
        exit 0
    fi
...
echo
echo "代码作者: https://github.com/MrAminiDev/"
echo "----------------------------------------"
echo
